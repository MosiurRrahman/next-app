...
const BlogPage = () => {
    ...
    const currentBlogs = blog_grid.slice(indexOfFirstBlog, indexOfLastBlog)
    ...
    useEffect(() => {
        ...
        link.rel = 'icon'
        link.href = '/assets/img/fav-icon.svg'
        document.head.appendChild(link)

        return () => {
            // Clean up the favicon when the component unmounts
    ...
    return (
        <div>
            ...
            <div className="blog-grid-page pt-150 mb-120" id="scroll-section">
                                ...
                                <div className="col-lg-4 col-md-6" key={index}>
                                    <div className="blog-card2 magnetic-item">
                                        <div className="blog-card-img-wrap">
                                            <Link href={`/blog/blog-details/${slugify(item.blog_title)}`} className="card-img">
                                                <img src={item.cover_image} alt="" />
                                            </Link>
                                        </div>
                                        <div className="card-content">
                                            <div className="blog-meta">
                                                <Link href="/blog/blog-sidebar" className="blog-date">
                                                    {item.date}
                                                </Link>
                                                <ul className="category">
                                                    <li><Link href="/blog/blog-sidebar">{item.category}</Link></li>
                                                </ul>
                                            </div>
                                            <h4><Link href={`/blog/blog-details/${slugify(item.blog_title)}`}>{item.blog_title}</Link></h4>
                                        </div>
                                    </div>
                                </div>
                            )
                        ...
                        <div className="col-lg-12 d-flex justify-content-center">
                            <div className="innerpage-pagination-area">
                                <ul className="paginations">
                                    ...
                                    {Array.from({ length: Math.ceil(blog_grid.length / blogsPerPage) }).map((_, index) => (
                                        <li key={index} className={`page-item ${currentPage === index + 1 ? 'active' : ''}`}>
                                            <a href="#" onClick={() => paginate(index + 1)}>{index + 1}</a>
                                        </li>
                                    ))}
                                    <li className="page-item paginations-button">
                                        <a href="#" onClick={() => paginate(currentPage < Math.ceil(blog_grid.length / blogsPerPage) ? currentPage + 1 : Math.ceil(blog_grid.length / blogsPerPage))}>
                                            <svg width={14} height={12} viewBox="0 0 14 12" xmlns="http://www.w3.org/2000/svg">
