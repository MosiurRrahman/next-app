...
const ProductDetails = ({ params }) => {
    ...
    const product = products.find(product => slugify(product.name) === decodedTitle)
    const [quantity, setQuantity] = useState(1)
    const [productCount, setProductCount] = useState(product.product_count || 1)

    if (!product) {
        console.log('Decoded title:', decodedTitle)
        console.log('Available product names:', products.map(product => slugify(product.name)))
    ...
    const incrementQuantity = () => {
        setQuantity(prevQuantity => {
            ...
            const updatedProducts = products.map(p =>
                p.id === product.id ? { ...p, product_count: newQuantity } : p
            )
    ...
    return (
        <div>
            ...
            <Breadcrum pageTitle={'Product Details'} pagename={'Product Details'} />
            <div className="product-details-top-section pt-150 mb-100" id="scroll-section">
                <div className="container-xl container-fluid-lg container">
                    <div className="row gy-5">
                        <div className="col-lg-6">
                            <div className="product-details-img">
                                <div className="tab-content" id="v-pills-tabContent">
                                    {product.image_details.map((image, index) => (
                                        <div key={index} className={`tab-pane fade ${index === 0 ? 'show active' : ''}`} id={`v-pills-img${index + 1}`} role="tabpanel">
                            ...
                            <div className="product-details-content">
                                ...
                                <div className="rating-review">
                                    <div className="rating">
                                        <div className="star">
                                            {[...Array(5)].map((_, i) => (
                                                <i key={i} className={`bi bi-star${i < product.rating ? '-fill' : ''}`} />
                                            ))}
                                        </div>
                                        <a href="#reviews">({product.total_reviews} customer review{product.total_reviews !== 1 ? 's' : ''})</a>
                                    </div>
                                </div>
                                <p>{product.description}</p>
                                <div className="price-area">
                                    <span>${product.price.toFixed(2)} {product.sale_price && <del>${product.sale_price.toFixed(2)}</del>}</span>
                                </div>
                                <div className="quantity-area">
                                    <h6>Quantity</h6>
                                    <div className="quantity-counter">
                                        <button onClick={decrementQuantity} className="quantity__minus"><i className="bx bx-minus" /></button>
                                        <input name="quantity" type="text" className="quantity__input" value={quantity} readOnly />
                                        <button onClick={incrementQuantity} className="quantity__plus"><i className="bx bx-plus" /></button>
                                    </div>

                                </div>
                                <div className="product-details-btn">
                                    <Link href="/shop/cart" className="primary-btn" data-text="Add To Cart"><span>Add To Cart</span></Link>
                                    <Link href="/shop/checkout" className="primary-btn2" data-text="Buy Now"><span>Buy Now</span></Link>
                                </div>
            ...
            <div className="product-details-description mb-120">
                                ...
                                <div className="tab-content" id="nav-tabContent">
                                    <div className="tab-pane fade show active" id="nav-description" role="tabpanel" aria-labelledby="nav-description-tab">
                                        <div className="description-content">
                                            <p>Canon cameras are a benchmark in the photography industry, known for their superb image quality, advanced features, and user-friendly design. Whether you're a beginner or a seasoned professional, Canon's range of cameras caters to every level of experience. The intuitive controls and ergonomic designs make it easy for users to get the most out of their photography, whether they are capturing everyday moments or professional projects.</p>
                                            <p>In addition to their performance, Canon cameras are highly versatile, supported by an extensive lineup of lenses and accessories that allow photographers to explore their creative potential. From wide-angle to telephoto, Canon lenses offer exceptional sharpness and detail, enabling photographers to shoot in various conditions and environments.</p>
                                            <div className="description-img-grp">
                                                <div className="row">
                                                    <div className="col-md-6">
                                                        <img src="assets/img/inner-pages/product-description-img1.jpg" alt="" />
                                                    </div>
                                                    <div className="col-md-6">
                                                        <img src="assets/img/inner-pages/product-description-img2.jpg" alt="" />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                                            ...
                                                            <div className="row">
                                                                ...
                                                                <div className="col-lg-12">
                                                                    <div className="form-inner two">
                                                                        <button className="primary-btn" type="submit" data-text="Submit"><span>Submit</span></button>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </form>
