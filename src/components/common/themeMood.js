...
const DarkMode = () => {
  useEffect(() => {
    ...
    (function () {
      ...
      if (!pref) {
        ...
        if (win.matchMedia(prefers_dark).matches) activateTheme(cls_dark);
        else activateTheme(cls_light);

        // real-time switching if supported by OS/browser
        win.matchMedia(prefers_dark).addListener(function (e) {
          if (e.matches) activateTheme(cls_dark);
        });
        win.matchMedia(prefers_light).addListener(function (e) {
          if (e.matches) activateTheme(cls_light);
        });
      }
  ...
  return (
    <>
      <div className="tt-style-switch dark-mode-toggle index-dark">
